...
4. Explore the four core questions:
   ...
   - How can you get it?
   - How will you deepen commitment and prepare for obstacles?
...
Load the relevant reference files as needed. The reference layer intentionally preserves the user's source notes in a fuller form; do not rely only on this short entry point for nuanced coaching.

- Read `references/01-principles-and-stance.md` for coaching assumptions, no-advice stance, powerful listening, clean questions, and tone.
- Read `references/00-source-reading-notes.md` when nuanced detail, original phrasing, chapter context, or a future extension requires the complete source notes.
- Read `references/02-rapport-listening-no-advice.md` for rapport, value-word listening, open questions, silence, scaling, and shifting from telling to asking.
- Read `references/03-contract-and-client-state.md` for setting the coaching contract, handling visitor/complainer/creator states, and defining a good contract.
- Read `references/04-four-core-questions.md` for the four engagement questions and their variations.
- Read `references/05-outcome-framework.md` for positive outcomes, control, SMART evidence, contingency planning, ecology checks, and visualization.
- Read `references/06-logical-levels.md` when the user is stuck, making a life/career choice, exploring interests, or confusing behavior with identity.
- Read `references/07-action-and-completion.md` when moving from insight to action, commitment, and session completion.
- Read `references/08-coach-self-awareness.md` for coach self-checks, non-projection, genuine empowerment, visualization language, and coach congruence.
- Read `references/09-session-tool-card.md` for a compact end-to-end coaching-session card.
- Read `references/10-question-bank.md` when you need precise coaching questions.
